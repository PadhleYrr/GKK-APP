...
// ── ADMIN STATE ──────────────────────────────────────────
let _isAdminCache = null; // null = not checked yet

// Call this to check if current user is admin (reads Firestore config/admins)
async function checkAndApplyAdminRole(email) {
  if (!email) { applyAdminUI(false); return; }
  try {
    const db = firebase.firestore();
    const snap = await db.collection('config').doc('admins').get();
    if (snap.exists) {
      const emails = snap.data().emails || [];
      _isAdminCache = emails.includes(email);
    } else {
      _isAdminCache = false;
    }
  } catch(e) {
...
// Show/hide admin-only UI elements based on role
function applyAdminUI(adminStatus) {
  // Review Flags nav item — only visible to admins
  const reviewNav = document.querySelector('[onclick="showPage(\'review\')"]');
  if (reviewNav) {
    reviewNav.style.display = adminStatus ? 'flex' : 'none';
  }
  // Admin panel link in account modal — re-render badge if open
  const badge = document.getElementById('user-badge');
  if (badge) updateUserBadge();
}
...
function initFirebase() {
  ...
  // Hide review nav by default until admin check completes
  const reviewNav = document.querySelector('[onclick="showPage(\'review\')"]');
  ...
  firebaseAuth.onAuthStateChanged(user => {
    if (user) {
      currentUser = user;
      onUserLoggedIn(user);
      checkAndApplyAdminRole(user.email); // async admin check
    } else {
      currentUser = null;
      _isAdminCache = null;
      applyAdminUI(false);
...
function showAccountModal() {
  ...
  el.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:28px;max-width:340px;width:100%;text-align:center">
      ...
      ${isAdmin() ? `<button onclick="window.open('admin.html','_blank');document.getElementById('account-modal').remove()" style="width:100%;padding:10px;background:#EEF2FF;color:#1A237E;border:1px solid #C7D7FD;border-radius:10px;font-size:13px;font-weight:600;cursor:pointer;margin-bottom:8px;font-family:'DM Sans',sans-serif">&#128295; Admin Panel</button>` : ''}
