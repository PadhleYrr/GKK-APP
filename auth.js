...
function switchTab(tab) {
  ...
  if (tab === 'login') {
    ...
    tabLogin.style.background = '#1A237E';
    tabLogin.style.color = '#fff';
    tabRegister.style.background = '#F1F5F9';
    tabRegister.style.color = '#374151';
    forgot.style.display = 'block';
  } else {
    nameField.style.display = 'block';
    btn.textContent = 'Create Account';
    tabRegister.style.background = '#1A237E';
    tabRegister.style.color = '#fff';
    tabLogin.style.background = '#F1F5F9';
    ...
    <div style="background:#fff;border-radius:20px;padding:28px;max-width:340px;width:100%;text-align:center">
      ...
      <div style="background:${premium ? '#F0FDF4' : daysLeft > 0 ? '#FFFBEB' : '#FEF2F2'};border-radius:12px;padding:14px;margin-bottom:16px">
        <div style="font-size:13px;font-weight:700;color:${premium ? '#15803D' : daysLeft > 0 ? '#D97706' : '#DC2626'}">
          ${premium ? `✅ Premium — ${daysLeft} days left` : daysLeft > 0 ? `⏳ Trial — ${daysLeft} days left` : '❌ Trial Expired'}
        </div>
        ${!premium ? `<div style="font-size:12px;color:#64748B;margin-top:4px">Unlock full access for ₹100 / 6 months</div>` : ''}
      </div>
      ${!premium ? `<button onclick="document.getElementById('account-modal').remove();openPayment()" style="width:100%;padding:12px;background:#1A237E;color:#fff;border:none;border-radius:10px;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:8px">🔓 Upgrade to Premium — ₹100</button>` : ''}
